...
async function getOverview(
  ...
  const response = await client.properties.runReport({
    property: propertyId,
    requestBody: {
      // GA4 returns one row per date range in request order, so formatOverview
      // matches positionally (rows[0] = current, rows[1] = previous).
      // `dateRange` cannot be listed in `dimensions` — GA4 rejects it.
...
function formatOverview(data: any) {
  ...
  type Row = { metricValues?: { value: string }[] }
  const current = (data.rows[0] as Row)?.metricValues || []
  const previous = (data.rows[1] as Row)?.metricValues || []
